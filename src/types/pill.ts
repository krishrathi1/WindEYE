...
/// Presentation mode for the collapsed pill, mirroring Apple's Dynamic Island:
/// - "compact": one activity, laid out inside the pill
/// - "minimal": two concurrent activities — the leading one stays in the pill and
///   the secondary one detaches into its own bubble beside it
export type CollapsedPresentation = "compact" | "minimal";
...
// Live Activity state — ongoing work with progress (downloads, transfers, and
// anything registered through useLiveActivities).
export interface LiveActivityState extends BaseContentState {
  type: "live_activity";
  data: {
    title: string;
    subtitle?: string;
...
export function createContentState<T extends ContentState>(
  ...
  const priorities: Record<ContentStateType, number> = {
    ...
    live_activity: STATE_PRIORITY.LIVE_ACTIVITY,
