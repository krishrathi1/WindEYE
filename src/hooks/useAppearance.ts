...
async function migrateLocalStorage(): Promise<Partial<AppearanceSettings> | null> {
  try {
    const stored = localStorage.getItem("windeye-appearance");
    if (stored) {
      const parsed = JSON.parse(stored);
      localStorage.removeItem("windeye-appearance");
      localStorage.removeItem("windeye-pill-mode");
      return parsed;
    }
    const oldMode = localStorage.getItem("windeye-pill-mode");
