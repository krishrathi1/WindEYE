...
export function useCrashRecovery(
  ...
  useEffect(() => {
    ...
    try {
      const saved = localStorage.getItem("windeye_crash_history");
      if (saved) {
  ...
  const saveCrashHistory = useCallback((history: CrashReport[]) => {
    try {
      localStorage.setItem("windeye_crash_history", JSON.stringify(history));
    } catch {
      // Ignore localStorage errors
    }
  }, []);
  ...
  const triggerRecovery = useCallback(async () => {
    ...
    try {
      ...
      const crashHistoryData = localStorage.getItem("windeye_crash_history");
      localStorage.clear();
      if (crashHistoryData) {
        localStorage.setItem("windeye_crash_history", crashHistoryData);
      }
  ...
  const clearCrashHistory = useCallback(() => {
    setCrashHistory([]);
    saveCrashHistory([]);
    localStorage.removeItem("windeye_crash_history");
  }, [saveCrashHistory]);
