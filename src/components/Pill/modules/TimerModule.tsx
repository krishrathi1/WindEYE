...
export function TimerExpanded({
  ...
  // Timer is running
  if (timer.isActive) {
    // Ring sized so the time fits fully inside; the label renders BELOW the ring
    // (it used to sit inside the circle and overflow across the stroke on longer
    // names like "Quick Focus", which made the ring look broken).
    const ringSize = 88;
    const strokeWidth = 4;
    ...
    return (
      <div className="flex flex-col items-center gap-1.5 py-1">
        ...
        <span className="text-[12px] text-white/85 max-w-[240px] truncate text-center">
          {timer.label}
        </span>
...
function TimerStartView({
  ...
  // Custom timer: user-entered label + minutes alongside the fixed presets.
  const [customLabel, setCustomLabel] = useState("");
  const [customMinutes, setCustomMinutes] = useState("25");

  const parsedMinutes = Math.floor(Number(customMinutes));
  const customValid = Number.isFinite(parsedMinutes) && parsedMinutes >= 1 && parsedMinutes <= 599;
  ...
  return (
    <div className="flex flex-col gap-2 py-1">
      ...
      {/* Custom timer: name + minutes + start */}
      <div className="flex items-center gap-1.5 justify-center px-2" role="group" aria-label="Custom timer">
        <input
          type="text"
          value={customLabel}
          ...
          className="flex-1 min-w-0 rounded-md bg-white/10 border border-white/15 px-2 py-1 text-[12px] text-white placeholder:text-white/35 focus:outline-none focus:ring-1 focus:ring-white/30"
        />
        <input
          type="number"
          value={customMinutes}
          ...
          className="w-14 rounded-md bg-white/10 border border-white/15 px-1.5 py-1 text-[12px] text-white text-center tabular-nums focus:outline-none focus:ring-1 focus:ring-white/30 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
        />
        <span className="text-[11px] text-white/60" aria-hidden="true">min</span>
        <motion.button
          ...
          aria-label={customValid ? `Start custom timer for ${parsedMinutes} minutes` : "Enter minutes between 1 and 599"}
          disabled={!customValid}
          {...(customValid ? microInteractions.button : {})}
          onClick={startCustom}
        >
          Start
        </motion.button>
