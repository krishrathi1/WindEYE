...
function SeekBar({ timeline, accentColor, onSeek, isPlaying = false }: SeekBarProps) {
  const [isDragging, setIsDragging] = useState(false);
  const [dragPosition, setDragPosition] = useState(0);
  const barRef = useRef<HTMLDivElement>(null);

  // The backend only reports a fresh position every ~1.5s poll, so the bar would
  // advance in visible jumps. Interpolate locally: snapshot the backend position
  // on each update, then tick a few times a second while playing so the fill
  // glides smoothly. On the next poll the base resets to the authoritative value,
  // keeping it accurate (no client-side drift accumulation).
  const baseRef = useRef({ positionMs: timeline.positionMs, at: performance.now() });
  const [nowTs, setNowTs] = useState(() => performance.now());
  useEffect(() => {
    baseRef.current = { positionMs: timeline.positionMs, at: performance.now() };
    setNowTs(performance.now());
  }, [timeline.positionMs]);
  useEffect(() => {
    if (!isPlaying || isDragging) return;
    const id = setInterval(() => setNowTs(performance.now()), 250);
  ...
  const livePositionMs =
    isPlaying && !isDragging
      ? Math.min(
          timeline.durationMs > 0 ? timeline.durationMs : Number.POSITIVE_INFINITY,
          baseRef.current.positionMs + (nowTs - baseRef.current.at)
        )
      : timeline.positionMs;
  ...
  const progress = isDragging
    ...
    ? livePositionMs / timeline.durationMs
    : 0;
