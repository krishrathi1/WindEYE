/**
 * Generate app icons from `public/windeye.svg` for:
 * - Web: `public/windeye-*.png` (optional, but handy)
...
const srcSvg = path.join(root, "public", "windeye.svg");
...
async function main() {
  ...
  // Web PNGs (optional)
  await renderPng(192, path.join(webOutDir, "windeye-192.png"));
  await renderPng(512, path.join(webOutDir, "windeye-512.png"));
  ...
  for (const s of sizes) {
    const p = path.join(tmpDir, `windeye-${s}.png`);
    await renderPng(s, p);
    pngPaths.push(p);
  }
  ...
  console.log(" - public/windeye.svg (favicon)");
  console.log(" - public/windeye-192.png");
  console.log(" - public/windeye-512.png");
}
